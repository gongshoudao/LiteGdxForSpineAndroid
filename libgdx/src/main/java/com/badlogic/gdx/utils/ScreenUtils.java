...
public final class ScreenUtils {
     ...
     */
    public static TextureRegion getFrameBufferTexture(Graphics graphics, GL20 gl) {
        final int w = graphics.getBackBufferWidth();
        final int h = graphics.getBackBufferHeight();
        return getFrameBufferTexture(gl, 0, 0, w, h);
    }
     ...
     */
    public static TextureRegion getFrameBufferTexture(GL20 gl, int x, int y, int w, int h) {
        final int potW = MathUtils.nextPowerOfTwo(w);
        final int potH = MathUtils.nextPowerOfTwo(h);

        final Pixmap pixmap = getFrameBufferPixmap(gl, x, y, w, h);
    ...
    public static Pixmap getFrameBufferPixmap(GL20 gl, int x, int y, int w, int h) {
     ...
     */
    public static byte[] getFrameBufferPixels(Graphics graphics, GL20 gl, boolean flipY) {
        final int w = graphics.getBackBufferWidth();
        final int h = graphics.getBackBufferHeight();
     ...
     * @param flipY whether to flip pixels along Y axis
     */
    public static byte[] getFrameBufferPixels(GL20 gl, int x, int y, int w, int h, boolean flipY) {
        gl.glPixelStorei(GL20.GL_PACK_ALIGNMENT, 1);
