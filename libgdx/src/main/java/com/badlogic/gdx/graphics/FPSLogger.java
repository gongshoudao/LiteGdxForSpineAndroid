...
public class FPSLogger {
	...
	public FPSLogger(Application app) {
        this(app, Integer.MAX_VALUE);
    }
     ...
     */
    public FPSLogger(Application app, int bound) {
		this.app = app;
		this.bound = bound;
    ...
    public void log() {
        ...
        if (nanoTime - startTime > 1000000000) /* 1,000,000,000ns == one second */ {
            final int fps = app.getGraphics().getFramesPerSecond();
            if (fps < bound) {
                app.log("FPSLogger", "fps: " + fps);
                startTime = nanoTime;
            }
        }
    }
