...
public class ParticleEffect implements Disposable {
    ...
    public void load(FileHandle effectFile, FileHandle imagesDir, Application app) {
        loadEmitters(effectFile);
        loadEmitterImages(imagesDir, app);
    }

    public void load(FileHandle effectFile, TextureAtlas atlas, Application app) {
        load(effectFile, atlas, null);
    }

    public void load(FileHandle effectFile, TextureAtlas atlas, String atlasPrefix, Application app) {
        loadEmitters(effectFile);
        loadEmitterImages(atlas, atlasPrefix, app);
    }
    ...
    public void loadEmitterImages(TextureAtlas atlas, Application app) {
        loadEmitterImages(atlas, null, app);
    }

    public void loadEmitterImages(TextureAtlas atlas, String atlasPrefix, Application app) {
        for (int i = 0, n = emitters.size; i < n; i++) {
            ParticleEmitter emitter = emitters.get(i);
    ...
    public void loadEmitterImages(FileHandle imagesDir, Application app) {
        ownsTexture = true;
        ...
        for (int i = 0, n = emitters.size; i < n; i++) {
            ...
            for (String imagePath : emitter.getImagePaths()) {
                ...
                if (sprite == null) {
                    sprite = new Sprite(loadTexture(imagesDir.child(imageName), app));
    ...
    protected Texture loadTexture(FileHandle file, Application app) {
        return new Texture(file, false, app);
    }
