...
public class InstanceBufferObjectSubData implements InstanceData {
    ...
    private int createBufferObject() {
        int result = graphics.getGL20().glGenBuffer();
        graphics.getGL20().glBindBuffer(GL20.GL_ARRAY_BUFFER, result);
        graphics.getGL20().glBufferData(GL20.GL_ARRAY_BUFFER, byteBuffer.capacity(), null, usage);
        graphics.getGL20().glBindBuffer(GL20.GL_ARRAY_BUFFER, 0);
        return result;
    }
    ...
    private void bufferChanged() {
        if (isBound) {
            graphics.getGL20().glBufferData(GL20.GL_ARRAY_BUFFER, byteBuffer.limit(), null, usage);
            graphics.getGL20().glBufferSubData(GL20.GL_ARRAY_BUFFER, 0, byteBuffer.limit(), byteBuffer);
            isDirty = false;
        }
    }
    ...
    public void bind(final ShaderProgram shader, final int[] locations) {
        final GL20 gl = graphics.getGL20();
        ...
        if (locations == null) {
            for (int i = 0; i < numAttributes; i++) {
                ...
                graphics.getGL30().glVertexAttribDivisor(location + unitOffset, 1);
            }
        } else {
            for (int i = 0; i < numAttributes; i++) {
                final VertexAttribute attribute = attributes.get(i);
