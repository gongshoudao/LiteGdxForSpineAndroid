...
public class Mesh implements Disposable {
    ...
    protected Mesh(VertexData vertices, IndexData indices, boolean isVertexArray, Graphics graphics) {
        this.vertices = vertices;
        this.indices = indices;
        this.isVertexArray = isVertexArray;
        this.graphics = graphics;

        addManagedMesh(graphics.getApp(), this);
    }
     ...
     * @param attributes  the {@link VertexAttribute}s. Each vertex attribute defines one property of a vertex such as position,
     */
    public Mesh(Graphics graphics, VertexDataType type, boolean isStatic, int maxVertices, int maxIndices, VertexAttribute... attributes) {
     ...
     */
    public Mesh(Graphics graphics, VertexDataType type, boolean isStatic, int maxVertices, int maxIndices, VertexAttributes attributes) {
        this.graphics = graphics;
        switch (type) {
    ...
    public void render(ShaderProgram shader, int primitiveType, int offset, int count, boolean autoBind) {
        ...
        if (isVertexArray) {
            if (indices.getNumIndices() > 0) {
                ...
                graphics.getGL20().glDrawElements(primitiveType, count, GL20.GL_UNSIGNED_SHORT, buffer);
                buffer.position(oldPosition);
                buffer.limit(oldLimit);
            } else {
                graphics.getGL20().glDrawArrays(primitiveType, offset, count);
            }
        } else {
            int numInstances = 0;
            ...
            if (indices.getNumIndices() > 0) {
                ...
                if (isInstanced && numInstances > 0) {
                    graphics.getGL30().glDrawElementsInstanced(primitiveType, count, GL20.GL_UNSIGNED_SHORT, offset * 2, numInstances);
                } else {
                    graphics.getGL20().glDrawElements(primitiveType, count, GL20.GL_UNSIGNED_SHORT, offset * 2);
                }
            } else {
                if (isInstanced && numInstances > 0) {
                    graphics.getGL30().glDrawArraysInstanced(primitiveType, offset, count, numInstances);
                } else {
                    graphics.getGL20().glDrawArrays(primitiveType, offset, count);
                }
            }
     ...
     */
    public void dispose() {
        if (meshes.get(graphics.getApp()) != null)
            meshes.get(graphics.getApp()).removeValue(this, true);
        vertices.dispose();
    ...
    public Mesh copy(boolean isStatic, boolean removeDuplicates, final int[] usage) {
        ...
        Mesh result;
        if (attrs == null)
            result = new Mesh(graphics, isStatic, numVertices, indices == null ? 0 : indices.length, getVertexAttributes());
        else
            result = new Mesh(graphics, isStatic, numVertices, indices == null ? 0 : indices.length, attrs);
