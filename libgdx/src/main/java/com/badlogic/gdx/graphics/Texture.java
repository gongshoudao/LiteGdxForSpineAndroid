...
public class Texture extends GLTexture {
    ...
    public Texture(FileHandle file, Application app) {
        this(file, null, false, app);
    }

    public Texture(FileHandle file, boolean useMipMaps, Application app) {
        this(file, null, useMipMaps, app);
    }

    public Texture(FileHandle file, Format format, boolean useMipMaps, Application app) {
        this(TextureData.Factory.loadFromFile(file, format, useMipMaps), app);
    }

    public Texture(Pixmap pixmap, Application app) {
        this(new PixmapTextureData(pixmap, null, false, false), app);
    }

    public Texture(Pixmap pixmap, boolean useMipMaps, Application app) {
        this(new PixmapTextureData(pixmap, null, useMipMaps, false), app);
    }

    public Texture(Pixmap pixmap, Format format, boolean useMipMaps, Application app) {
        this(new PixmapTextureData(pixmap, format, useMipMaps, false), app);
    }

    public Texture(int width, int height, Format format, Application app) {
        this(new PixmapTextureData(new Pixmap(width, height, format), null, false, true), app);
    }

    public Texture(TextureData data, Application app) {
        this(app, GL20.GL_TEXTURE_2D, app.getGraphics().getGL20().glGenTexture(), data);
    }

    protected Texture(Application app, int glTarget, int glHandle, TextureData data) {
        super(app.getGraphics(), glTarget, glHandle);
        this.app = app;
        load(data);
        if (data.isManaged()) addManagedTexture(app, this);
    }
    ...
    public void load(TextureData data) {
        ...
        graphics.getGL20().glBindTexture(glTarget, 0);
    }
    ...
    protected void reload() {
        ...
        glHandle = graphics.getGL20().glGenTexture();
        load(data);
    }
    ...
    public void draw(Pixmap pixmap, int x, int y) {
        ...
        bind();
        graphics.getGL20().glTexSubImage2D(glTarget, 0, x, y, pixmap.getWidth(), pixmap.getHeight(), pixmap.getGLFormat(), pixmap.getGLType(),
                pixmap.getPixels());
    }
    ...
    public void dispose() {
        ...
        delete();
        if (data.isManaged() && app != null) if (managedTextures.get(app) != null)
            managedTextures.get(app).removeValue(this, true);
    }
        ...
        if (assetManager == null) {
            ...
            for (Texture texture : textures) {
                ...
                if (fileName == null) {
                    ...
                    texture.glHandle = app.getGraphics().getGL20().glGenTexture();
