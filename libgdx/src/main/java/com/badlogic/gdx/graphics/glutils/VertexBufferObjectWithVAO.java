...
public class VertexBufferObjectWithVAO implements VertexData {
    ...
    private void bufferChanged() {
        if (isBound) {
            graphics.getGL20().glBufferData(GL20.GL_ARRAY_BUFFER, byteBuffer.limit(), byteBuffer, usage);
            isDirty = false;
        }
    }
    ...
    private void bindAttributes(ShaderProgram shader, int[] locations) {
        ...
        if (!stillValid) {
            graphics.getGL20().glBindBuffer(GL20.GL_ARRAY_BUFFER, bufferHandle);
    ...
    public void invalidate() {
        bufferHandle = graphics.getGL30().glGenBuffer();
        createVAO();
        isDirty = true;
    }
    ...
    private void createVAO() {
        tmpHandle.clear();
        graphics.getGL30().glGenVertexArrays(1, tmpHandle);
    ...
    private void deleteVAO() {
        if (vaoHandle != -1) {
            ...
            tmpHandle.flip();
            graphics.getGL30().glDeleteVertexArrays(1, tmpHandle);
            vaoHandle = -1;
        }
    }
}
