...
public class TextureAtlas implements Disposable {
     ...
     */
    public TextureAtlas(FileHandle packFile, Application app) {
        this(packFile, packFile.parent(), app);
    }

    /**
     * @param flip If true, all regions loaded will be flipped for use with a perspective where 0,0 is the upper left corner.
     * @see #TextureAtlas(FileHandle, Application)
     */
    public TextureAtlas(FileHandle packFile, boolean flip, Application app) {
        this(packFile, packFile.parent(), flip, app);
    }

    public TextureAtlas(FileHandle packFile, FileHandle imagesDir, Application app) {
        this(packFile, imagesDir, false, app);
    }

    /**
     * @param flip If true, all regions loaded will be flipped for use with a perspective where 0,0 is the upper left corner.
     */
    public TextureAtlas(FileHandle packFile, FileHandle imagesDir, boolean flip, Application app) {
        this(new TextureAtlasData(packFile, imagesDir, flip), app);
    }
     ...
     * @param app
     */
    public TextureAtlas(TextureAtlasData data, Application app) {
        if (data != null) load(data, app);
    }

    private void load(TextureAtlasData data, Application app) {
        ObjectMap<Page, Texture> pageToTexture = new ObjectMap<Page, Texture>();
        for (Page page : data.pages) {
            Texture texture = null;
            if (page.texture == null) {
                texture = new Texture(page.textureFile, page.format, page.useMipMaps, app);
