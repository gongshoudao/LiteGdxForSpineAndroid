...
public class ShaderProgram implements Disposable {
    ...
    public ShaderProgram(Graphics graphics, String vertexShader, String fragmentShader) {
        ...
        if (isCompiled()) {
            fetchAttributes();
            fetchUniforms();
            addManagedShader(graphics.getApp(), this);
        }
    }
...
// }
    ...
    private int linkProgram(int program) {
        ...
        if (linked == 0) {
// graphics.getGL20().glGetProgramiv(program, GL20.GL_INFO_LOG_LENGTH, intbuf);
// int infoLogLength = intbuf.get(0);
// if (infoLogLength > 1) {
            log = graphics.getGL20().glGetProgramInfoLog(program);
// }
            return -1;
        }
...
// }
    ...
    public int fetchUniformLocation(String name, boolean pedantic) {
        ...
        if ((location = uniforms.get(name, -2)) == -2) {
            location = graphics.getGL20().glGetUniformLocation(program, name);
    ...
    public void dispose() {
        ...
        if (shaders.get(graphics.getApp()) != null)
            shaders.get(graphics.getApp()).removeValue(this, true);
    }
    ...
    public static void invalidateAllShaderPrograms(Application app) {
        if (app.getGraphics().getGL20() == null) return;
     ...
     */
    public static int getNumManagedShaderPrograms(Application app) {
        return shaders.get(app).size;
    }
    ...
    private void fetchUniforms() {
        params.clear();
        graphics.getGL20().glGetProgramiv(program, GL20.GL_ACTIVE_UNIFORMS, params);
        ...
        for (int i = 0; i < numUniforms; i++) {
            ...
            params.put(0, 1);
            type.clear();
            String name = graphics.getGL20().glGetActiveUniform(program, i, params, type);
            int location = graphics.getGL20().glGetUniformLocation(program, name);
            uniforms.put(name, location);
    ...
    private void fetchAttributes() {
        params.clear();
        graphics.getGL20().glGetProgramiv(program, GL20.GL_ACTIVE_ATTRIBUTES, params);
        ...
        for (int i = 0; i < numAttributes; i++) {
            ...
            params.put(0, 1);
            type.clear();
            String name = graphics.getGL20().glGetActiveAttrib(program, i, params, type);
            int location = graphics.getGL20().glGetAttribLocation(program, name);
            attributes.put(name, location);
