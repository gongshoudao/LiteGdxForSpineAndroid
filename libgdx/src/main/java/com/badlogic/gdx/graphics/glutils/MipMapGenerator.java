...
public class MipMapGenerator {
    ...
    private static void generateMipMapGLES20(Graphics graphics, int target, Pixmap pixmap) {
        graphics.getGL20().glTexImage2D(target, 0, pixmap.getGLInternalFormat(), pixmap.getWidth(), pixmap.getHeight(), 0,
                pixmap.getGLFormat(), pixmap.getGLType(), pixmap.getPixels());
        graphics.getGL20().glGenerateMipmap(target);
    }

    private static void generateMipMapCPU(Graphics graphics, int target, Pixmap pixmap, int textureWidth, int textureHeight) {
        graphics.getGL20().glTexImage2D(target, 0, pixmap.getGLInternalFormat(), pixmap.getWidth(), pixmap.getHeight(), 0,
                pixmap.getGLFormat(), pixmap.getGLType(), pixmap.getPixels());
        if ((graphics.getGL20() == null) && textureWidth != textureHeight)
        ...
        while (width > 0 && height > 0) {
            ...
            graphics.getGL20().glTexImage2D(target, level, pixmap.getGLInternalFormat(), pixmap.getWidth(), pixmap.getHeight(), 0,
                    pixmap.getGLFormat(), pixmap.getGLType(), pixmap.getPixels());
